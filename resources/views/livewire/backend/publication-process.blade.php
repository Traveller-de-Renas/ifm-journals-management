<x-module>
    ...
    <div class="{{ ($filters)? 'block' : 'hidden'}}" >
        <div class="bg-white dark:bg-slate-800 rounded-sm border border-slate-200 dark:border-slate-700 cetne ccg4t ctk06 mb-6">
        ...
        @foreach ($data as $row)
            <div class="border border-slate-200 dark:border-slate-700 p-4 shadow-md mb-4 rounded-md">
                ...
                <div class="flex items-top">
                    ...
                    @else
                        <img class="c7n6y" src="{{ asset('storage/journals/'.$row->image) }}" width="60" height="80" alt="{{ strtoupper($row->code) }}">
                    @endif
                    <div class="">
                ...
                <a href="{{ route('journals.archive', $row->uuid) }}">
                    <x-button >Go to Publication </x-button>
                </a>
