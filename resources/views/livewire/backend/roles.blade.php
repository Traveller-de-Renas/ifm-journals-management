            ...
            <tr class="border-b transition duration-300 ease-in-out hover:bg-neutral-100 grey:border-neutral-500 grey:hover:bg-neutral-600">
                ...
                <td class="whitespace-nowrap px-6 py-4 text-wrap text-justify">
                    @foreach ($item->permissions as $permission)
                        <span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded">{{ $permission->name }}</span>
                    @endforeach
                </td>
    ...
    <x-dialog-modal wire:model="View">
        ...
        <x-slot name="content">
            ...
            <p class="text-lg text-gray-950 mt-6">
                Note : Click on a specific permission to remove that permission from this role
            </p>

            <div class="mt-4">
                @if ($role)
                    @foreach ($role->permissions as $permission)
                        <x-button type="submit" wire:click="remove_permission({{ $permission }})" wire:loading.attr="disabled">
                            {{ $permission->name }}
                        </x-button>
                    @endforeach
                @endif
        ...
        <x-slot name="footer">
            ...
            <x-button type="submit" wire:click="give_permission({{ $record }})" wire:loading.attr="disabled">
                {{ __('Submit') }}
            </x-button>
