            ...
            @if (in_array(auth()->user()->id, $teditor) || auth()->user()->hasRole('Administrator'))
                ...
                @foreach ($record->article_journal_users()->orderBy('number', 'ASC')->get() as $key => $article_user)
                    <span class="hover:text-blue-600 hover:underline cursor-pointer mr-2">
                        ...
                        @if ($article_user->user->salutation)
                            {{ $article_user->user->salutation?->title }}.
                        @endif

                        {{ $article_user->user->last_name }},
                        {{ strtoupper(substr($article_user->user->first_name, 0, 1)) }}.

                        @if ($article_user->user->affiliation)
                            ({{ $article_user->user->affiliation }})
                        @endif
            ...
            <div class="col-span-12">
                @php
                    if (in_array(auth()->user()->id, $teditor) || auth()->user()->hasRole('Administrator')) {
                        ...
                        $files_collect = $record
                            ?->files()
                            ->whereHas('file_category', function ($query) {
                        ...
                        @foreach ($files_collect as $key => $file)
                            <div class="grid grid-cols-12  bg-gray-200 rounded-lg ">
                                ...
                                <div class="col-span-8 p-2 px-4">
                                    <a href="{{ asset('storage/articles/' . $file->file_path) }}"><span
                                            class="font-bold text-blue-500 hover:text-blue-700 cursor-pointer ml-4">{{ $file->file_description }}</span></a>
                                </div>
        ...
        @if ($record->article_status->code == '014')
            <div class="w-full mb-8 grid grid-cols-12 gap-2">
                <div class="col-span-8">
                    ...
                    <span class="hover:text-blue-600 hover:underline cursor-pointer">{{ $record->author->last_name }},
                        {{ strtoupper(substr($record->author->first_name, 0, 1)) }}.
                    </span>
                    @foreach ($coauthors as $key => $user)
                        <span class="hover:text-blue-600 hover:underline cursor-pointer">{{ $user->last_name }},
                            {{ strtoupper(substr($user->first_name, 0, 1)) }}. </span>
                    @endforeach
                    ...
                    <span class="font-bold italic hover:text-blue-600 hover:underline cursor-pointer"><a
                            href="{{ route('journal.detail', $record?->journal->uuid) }}">{{ __($record->journal?->title) }}</a></span>,
                    {{ $record->issue?->volume->description }}, {{ $record->issue?->description }},
                    {{ $record->issue?->volume->journal->issn }}. {{ $record->issue?->volume->journal->doi }}
                </div>
            </div>
        @endif
    </div>
